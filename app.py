...
def build_prompt(q, matches, mode):
    style = {"concise":"Answer in 5–10 sentences.",
             "executive":"Bullet points for an executive.",
             "legal":"Quote exact lines; be neutral."}.get(mode, "Answer in 5–10 sentences.")
    ctx_lines = []
...
def to_citations(matches):
    ...
    for m in matches[:5]:
        md = m.get("metadata", {})
        out.append(Cit(doc_id=md.get("doc_id",""),
                       title=md.get("title"),
...
def ask(req: AskRequest):
    ...
    matches = [{"score":m.score, "metadata":m.metadata or {}} for m in res.matches]
    if not matches: return AskResponse(answer="No context found.", citations=[])
